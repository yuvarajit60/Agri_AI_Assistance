...
- **Phase 4 — partial, out of order**: the vector DB + knowledge base ingestion piece exists (BGE-M3 + Qdrant, but scoped to organic disease guidance only, not a general AI Advisor). Multi-language support in the app is **done** (English/Tamil, live-switchable) — well ahead of schedule. **AI Chat is now real** (`backend/services/ai_chat` — a single grounded Claude Opus 4.8 call per question, not a multi-step agent loop, same design choice as the LogicApp Monitor Agent this pattern is modeled on; grounded in the farm's real dashboard context — crop pick, weather, land health, water, market — assembled client-side and sent with the question, not re-fetched server-side; uses `client.messages.stream()` + adaptive thinking + `effort: medium` purely for timeout safety, still a single plain JSON response to the app, no SSE plumbed through). Deliberately lightweight (no embedding model, unlike `disease_kb`) so it fits Render's free tier. Replaces the old keyword-matched canned-reply screen. AI Advisor (tool-calling, proactive daily/weekly synthesis) still doesn't exist — the natural next step now that a single-call grounded pattern is proven here too.
...
1. **Deploy `ai_chat` to Render** (Blueprint sync + manual `ANTHROPIC_API_KEY`/`AI_CHAT_SERVICE_URL` entries) — built, tested locally (honest-degrade path verified end-to-end), not yet live. Lightweight, should deploy cleanly on the free tier.
...
5. Build the AI Advisor Service (Phase 4) — proactive daily/weekly synthesized task list, likely with real tool-calling (weather/market lookups mid-conversation) now that a single-call grounded chat pattern is proven in `ai_chat`.
